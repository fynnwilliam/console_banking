...
auto bank::open_account(account_builder &&builder) noexcept {
  unsigned id{new_id()};
  builder.email(email()).balance(amount()).id(id).type(account_t::savings);
...
void bank::open_account() noexcept {
  if (auto const &acc =
          open_account(account::create(last_name(), first_name())))
    std::cout << *acc;
  else
...
void bank::check_balance() const noexcept { check_balance(id()); }
...
void bank::deposit() noexcept { deposit(id(), amount()); }
...
void bank::withdraw() noexcept { withdraw(id(), amount()); }
...
void bank::close_account() noexcept { close_account(id()); }
...
void bank::list_transactions() const noexcept { list_transactions(id()); }
...
int bank::option() const noexcept {
  std::string s{request("\tselect an option, 0..7")};
  return s.size() == 1 && s[0] >= '0' && s[0] < '8' ? atoi(s.c_str())
                                                    : option();
}
...
std::string bank::request(std::string statement) const noexcept {
  ...
  return trim(input).size() ? input : request(std::move(statement));
}
